    ...
    static boolean isIndexBackendsSupportsListPropertyOrdering(JanusGraphTransaction tx) {
        ...
        while (configIt.hasNext()) {
            ...
            if (configKey.matches("index\\.[^.]+\\.backend")) {
                String configValue = tx.configuration().getString(configKey);
                if (configValue != null) {
                    String finalConfigValue = configValue.toLowerCase().trim();

                    if (!finalConfigValue.isEmpty()) {
                    ...
                    if (finalConfigValue.equals("lucene")) {
                        return false;
                    }
                }
            }
